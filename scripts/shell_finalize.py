...
def remove_premain_headers(text):
    ...
    prefix=text[:end]; suffix=text[end:]
...
for path in sorted(Path('.').glob('*.html')):
    ...
    else: print('WARNING: no body tag in',path.name)
    if '</head>' in text: text=text.replace('</head>',ASSETS+'\n</head>',1)
    else: print('WARNING: no head close in',path.name)
    ...
    if text!=original:
        path.write_text(text,encoding='utf-8'); print('Finalized shared shell:',path.name)
