...
for path in sorted(Path('.').glob('*.html')):
    ...
    text=path.read_text(encoding='utf-8'); markup=SCRIPT_BLOCK.sub('',STYLE_BLOCK.sub('',text)); dom_classes=set()
    ...
    for block in STYLE_BLOCK.findall(text):
        for selectors,decl in RULE.findall(block):
            ...
            rules_seen+=1; meaningful=(classes & dom_classes)-STRUCTURAL; governed=meaningful & LOCK_CLASSES
            if meaningful and not governed:
                ...
                for value in RADIUS.findall(decl):
                    v=value.strip().lower()
                    if v not in {'0','0px','2px','2px!important','50%','0!important'} and 'var(' not in v: rounded.append((selectors[:100],value.strip()[:60]))
                for value in SHADOW.findall(decl):
                    if value.strip().lower() not in {'none','none!important'}: shadows.append((selectors[:100],value.strip()[:80]))
                for value in FONT.findall(decl):
                    v=value.strip()
                    if not any(x in v.lower() for x in ['arial','helvetica','georgia','times new roman','serif','sans-serif']): fonts.append((selectors[:100],v[:80]))
    uncovered=uniq(uncovered); rounded=uniq(rounded,15); shadows=uniq(shadows,15); fonts=uniq(fonts,10); score=len(uncovered)+2*len(old_colors)+len(rounded)+len(shadows)+2*len(fonts); rows.append((score,path.name,rules_seen,len(uncovered),sorted(old_colors),rounded,shadows,fonts)); page_details[path.name]=uncovered
