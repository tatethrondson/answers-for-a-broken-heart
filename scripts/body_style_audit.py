...
ALLOWED_HEX = {x.lower() for x in {
    '#294533','#183024','#738371','#f5f0e7','#fbf8f2','#fffefb','#242a26','#656d67','#ad823d',
    '#ded8cd','#244833','#244432','#334139','#25382d','#2b5540','#264b39','#29332d','#2d3b33',
    '#283b30','#343a36','#3d4540','#3e4741','#3f4642','#252b27','#333333','#333','#393b39',
    '#6a655e','#6b736f','#8b6939','#88683b','#5f6862','#657068','#59645d','#5d665f','#525c56',
    '#4b5850','#48564d','#eef2ed','#eaf0e8','#f7f2e9','#f8f5ef','#faf8f3','#f1eadf','#f0ece4',
    '#eee7da','#eee8de','#f5f1e9','#fff8fa','#f3dce4','#5d464b','#fbf7f0','#c6a982','#d8bd87',
    '#ebe5da','#e3ddd2','#cfc6b7','#dad4c8','#d7d0c5','#ece6dd','#dde5dc','#d7dfd6','#d9d4ca',
    '#17291f','#2c503a','#274a36','#2b503b','#f6f4ed','#cbc6bc','#ccc5b9','#d9d4cb','#f8f5ee',
    '#eee8df','#e6e0d7','#ddd7cd','#6a716d','#777f79','#4d5751','#555e58','#676f6a','#d8d2c8',
    '#2d332f','#5b655f','#f8f4ec','#fffaf2','#e8efe8','#e4ded3','#fff'
}}
...
for path in sorted(Path('.').glob('*.html')):
    ...
    markup=STYLE_BLOCK.sub('', text); markup=SCRIPT_BLOCK.sub('', markup)
    dom_classes=set()
    ...
    for block in STYLE_BLOCK.findall(text):
        for selectors,decl in RULE.findall(block):
            ...
            if classes and not (classes & dom_classes): continue
            rules_seen += 1
            ...
            if meaningful and not governed:
                ...
                for value in SHADOW.findall(decl):
                    v=value.strip().lower()
                    if v not in {'none','none!important'}: shadows.append((selectors[:100],value.strip()[:80]))
    ...
    rows.append((score,path.name,rules_seen,len(uncovered),sorted(old_colors),rounded,shadows,fonts)); page_details[path.name]=uncovered

rows.sort(reverse=True)
report=['# Body Visual Consistency Audit','',
