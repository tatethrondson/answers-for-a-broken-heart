...
def check_http(url, want_body=False):
    ...
    try:
        ...
        req=Request(url,method=method,headers={'User-Agent':'Mozilla/5.0 LinkAudit/3.0'})
        ...
        if not want_body and status in (403,405,429):
            try:
                req=Request(url,headers={'User-Agent':'Mozilla/5.0 LinkAudit/3.0','Range':'bytes=0-0'})
                with urlopen(req,timeout=12) as r:
...
for source,parser in parsed.items():
    for kind,raw in parser.links:
        ...
        if kind!='form':
            ...
            repo_live_sources.setdefault(live_url,[]).append((source,kind,raw))
...
# Crawl the markup that is actually live, so stale deployments/old hrefs are visible.
...
for filename in sorted(pages):
    ...
    url=LIVE_BASE+route
    status,final,note,body=check_http(url,want_body=True)
    if status is None or status>=400:
        live_page_failures.append((url,status,note)); continue
    live_pages_fetched+=1
    parser=parse_text(body)
    for kind,raw in parser.links:
        raw=raw.strip()
        if not raw or raw.startswith(SKIP_SCHEMES) or kind=='form': continue
        absolute=urljoin(final,raw)
        u=urlparse(absolute)
        if u.scheme not in ('http','https'): continue
        if (u.hostname or '').lower() in SITE_HOSTS:
            clean=u._replace(fragment='').geturl()
            live_markup_sources.setdefault(clean,[]).append((route,kind,raw))
...
lines=['# Full Site Link Audit','',f'HTML pages scanned in repository: {len(pages)}',f'Link/resource/form occurrences checked in repository: {checked_occurrences}',f'Unique repo-derived live internal destinations checked: {len(repo_live_results)}',f'Live HTML pages fetched: {live_pages_fetched}',f'Unique destinations found in live page markup: {len(live_markup_results)}',f'Unique external URLs checked: {len(external_results)}','',f'Broken internal page links in repository: {len(internal_broken)}',f'Broken page fragments: {len(fragment_broken)}',f'Missing linked resources in repository: {len(resource_broken)}',f'Broken repo-derived production destinations: {len(repo_live_broken)}',f'Live pages that failed to load: {len(live_page_failures)}',f'Broken destinations found in live markup: {len(live_markup_broken)}',f'Broken/unreachable external URLs: {len(external_broken)}',f'Externally blocked/rate-limited checks: {len(external_warnings)}','']
...
section('Broken repo-derived production destinations',repo_live_broken,lambda r:f'`{r[0]}` — status `{r[1]}` {r[3]} — referenced by '+', '.join(sorted({x[0] for x in r[4]})))
...
print('\n'.join(lines[:17]))
if internal_broken or fragment_broken or resource_broken or repo_live_broken or live_page_failures or live_markup_broken:
    raise SystemExit(2)
